function [U] = cauchy_green_interpolate(V,T,W)
  % Compute complex linear combination deformation of vertices V, using
  % transformations at some control points T, propogated to the mesh using
  % weights W.
  %
  % [U] = cauchy_green_interpolate(V,T,W)
  % 
  % Inputs:
  ...
  VVV = reshape(complex(VVV(:,1,:), VVV(:,2,:)),[size(V,1),m]);
  % added weighted combinations of deformations according to each control
  U_complex = sum(W.*VVV,2);
  U = [real(U_complex) imag(U_complex)];
